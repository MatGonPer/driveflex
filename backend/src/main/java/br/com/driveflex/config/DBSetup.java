...
public class DBSetup {
    ...
    private static void criarTabelas(Connection conn) throws SQLException {
        ...
        try (Statement stmt = conn.createStatement()) {
            ...
            // Garantir que a coluna driver_id da tabela existente seja anulável
            try {
                stmt.execute("ALTER TABLE contracts ALTER COLUMN driver_id DROP NOT NULL;");
            } catch (SQLException ignored) {}
            
            // Adicionar novas colunas se não existirem
            try {
                stmt.execute("ALTER TABLE contracts ADD COLUMN IF NOT EXISTS pickup_location VARCHAR(255);");
                stmt.execute("ALTER TABLE contracts ADD COLUMN IF NOT EXISTS dropoff_location VARCHAR(255);");
                stmt.execute("ALTER TABLE contracts ADD COLUMN IF NOT EXISTS estimated_fare DOUBLE PRECISION;");
            } catch (SQLException ignored) {}
