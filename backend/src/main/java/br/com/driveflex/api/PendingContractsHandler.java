...
public class PendingContractsHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            // 3. Buscar usuário autenticado no banco
            ...
            // 4. Retornar contratos pendentes baseados na role do usuário
            if ("DRIVER".equalsIgnoreCase(user.getRole())) {
                // Motoristas veem todas as solicitações globais disponíveis para aceite
                pendingContracts = contractRepository.findAllPending();
            } else {
                // Passageiros/Clientes veem apenas as suas próprias solicitações de contrato ativas
                pendingContracts = contractRepository.findPendingByClientId(driverId);
            }
