...
public class AcceptContractHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            // 6. Atualizar o motorista e o status do contrato (aceite)
            boolean updated = contractRepository.acceptContractByDriver(contractId, driverId);
            if (!updated) {
                sendResponse(exchange, 500, "{\"error\": \"Falha ao aceitar contrato pelo motorista.\"}");
                return;
            }
