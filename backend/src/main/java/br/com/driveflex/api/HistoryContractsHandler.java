...
/**
 * Handler responsável por listar o histórico de todos os contratos de um usuário.
 * Rota esperada: GET /api/contracts/history
 */
public class HistoryContractsHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            // O histórico traz todos os contratos associados ao usuário, independente de role ou status.
            List<Contract> historyContracts = contractRepository.findAllByUserId(userId);

            String jsonResponse = gson.toJson(historyContracts);
            sendResponse(exchange, 200, jsonResponse);

        } catch (Exception e) {
            e.printStackTrace();
            sendResponse(exchange, 500, "{\"error\": \"Erro interno no servidor ao listar histórico de contratos: " + e.getMessage() + "\"}");
        }
    }
