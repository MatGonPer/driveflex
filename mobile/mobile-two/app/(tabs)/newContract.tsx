...
export default function NewContractScreen() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!token) {
        if (Platform.OS === 'web') {
          alert('Você precisa estar logado para solicitar um contrato.');
        } else {
          Alert.alert('Erro', 'Você precisa estar logado para solicitar um contrato.');
        }
      ...
      const startTime = `${dateStr}T${formatTime(pickupTime)}`;
      const endTime = returnTime ? `${dateStr}T${formatTime(returnTime)}` : null;

      let estimatedFare = 450.0;
      if (vehicleType === 'moto') estimatedFare = 300.0;
      if (vehicleType === 'van') estimatedFare = 800.0;
      ...
      console.log('Enviando solicitação para o backend:', payload);
      ...
      if (Platform.OS === 'web') {
        alert('Solicitação criada com sucesso! Aguardando aceite de um motorista.');
      } else {
        Alert.alert('Solicitação Criada', 'Aguardando aceite de um motorista.');
      }
      ...
      if (Platform.OS === 'web') {
        alert('Erro ao criar solicitação: ' + msg);
      } else {
        Alert.alert('Erro', 'Erro ao criar solicitação: ' + msg);
      }
    }
  };
