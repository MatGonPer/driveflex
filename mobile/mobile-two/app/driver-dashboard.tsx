...
export default function DriverDashboard() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      void refreshPending();
      void refreshActive();
    }, [refreshProfile, refreshPending, refreshActive])
  );

  const handleRefresh = async () => {
    setIsRefreshing(true);
    try {
      await Promise.all([refreshProfile(), refreshPending(), refreshActive()]);
    } finally {
  ...
  const renderContractsSection = () => {
    ...
    const currentError = isPending ? pendingError : activeError;
    const currentContracts = isPending ? pendingContracts : activeContracts;
    const currentLoading = isPending ? pendingLoading : activeLoading;

    return (
      <View style={styles.sectionContainer}>
        <View style={styles.tabContainer}>
          <TouchableOpacity 
            style={[styles.tabButton, isPending && styles.tabButtonActive]}
            onPress={() => setActiveTab('PENDING')}
          >
            <ThemedText style={[styles.tabButtonText, isPending && styles.tabButtonTextActive]}>
              Solicitações ({pendingContracts.length})
            </ThemedText>
          </TouchableOpacity>
          <TouchableOpacity 
            style={[styles.tabButton, !isPending && styles.tabButtonActive]}
        ...
        ) : currentContracts.length === 0 && !currentLoading ? (
          <ThemedView style={styles.emptyCard}>
            <Ionicons name={isPending ? "document-text-outline" : "checkmark-circle-outline"} size={40} color="#666666" />
            <ThemedText style={styles.emptyText}>
              {isPending ? "Nenhuma solicitação de contrato" : "Nenhum contrato ativo"}
            </ThemedText>
            <ThemedText style={styles.emptySubtext}>
              {isPending ? "Novos contratos aparecerão aqui" : "Aceite contratos para vê-los aqui"}
            </ThemedText>
          </ThemedView>
        ) : currentLoading ? (
